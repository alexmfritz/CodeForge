...
function evaluateHtmlAssertion(doc: Document, source: string, tc: TestCase): TestResult {
  ...
  switch (assertion) {
    case 'exists': {
      ...
      return { pass: el !== null, description, got: el !== null ? 'found' : `no element matching "${query}"` };
    }
    case 'textContains': {
      const el = query ? doc.querySelector(query) : null;
      const text = el?.textContent ?? '';
      const expected = String(value ?? '');
      return { pass: text.includes(expected), description, got: text.trim() };
    }
    case 'countAtLeast': {
    ...
    case 'hasId': {
      ...
      return { pass: actualId === String(value ?? ''), description, got: actualId || '(no id)' };
    }
    case 'hasClass': {
      const el = query ? doc.querySelector(query) : null;
      const hasIt = el?.classList.contains(String(value ?? '')) ?? false;
      return { pass: hasIt, description, got: hasIt ? 'has class' : `classes: ${el?.className ?? '(none)'}` };
    }
    case 'sourceContains': {
    ...
    case 'sourceMatch': {
      ...
      return { pass: matched, description, got: matched ? 'matched' : `pattern /${pattern}/${flags ?? 'i'} not found` };
    }
    default:
