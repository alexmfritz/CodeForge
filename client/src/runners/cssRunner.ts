...
export function runCssTests(
  ...
  return new Promise((resolve) => {
    ...
    doc.open();
    doc.write(
      `<!DOCTYPE html><html><head><style>${cssCode}</style></head><body>${providedHtml}</body></html>`,
    );
    doc.close();
...
function evaluateCssAssertion(doc: Document, source: string, tc: TestCase): TestResult {
  ...
  if (assertion === 'sourceContains') {
    ...
    return { pass: found, description, got: found ? 'found' : `"${needle}" not found in source` };
  }
  ...
  if (assertion === 'sourceMatch') {
    ...
    return { pass: matched, description, got: matched ? 'matched' : `pattern /${pattern}/${tc.flags ?? 'i'} not found` };
  }
