...
export default function TestResults({ results, isRunning }: TestResultsProps) {
  ...
  if (results.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-24 gap-2" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="flex items-center gap-3 pb-3 mb-1" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <div className="flex-1 h-1.5 rounded-full overflow-hidden" style={{ backgroundColor: 'var(--bg-raised)' }}>
          <div
            ...
            style={{ width: `${(passCount / results.length) * 100}%`, backgroundColor: allPass ? 'var(--success)' : 'var(--error)' }}
          />
        </div>
      </div>
      {results.map((r, i) => (
        ...
        >
          <span className="flex-shrink-0 mt-0.5 font-bold text-xs" style={{ color: r.pass ? 'var(--success)' : 'var(--error)' }}>
            {r.pass ? (
          ...
          <div className="flex-1 min-w-0">
            <span style={{ color: r.pass ? 'var(--text-secondary)' : 'var(--text-primary)' }}>{r.description}</span>
            {!r.pass && r.got !== undefined && (
              <div className="mt-1 text-xs px-2 py-0.5 rounded font-code" style={{ color: 'var(--error)', backgroundColor: 'rgba(248,113,113,0.08)' }}>
